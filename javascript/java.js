...
function createGif(input, amount){
    ...
    //var query = 'https://api.giphy.com/v1/gifs/search?q=' +input + '&rating=g&api_key=ihqeBjzMFNuSM45AZMn5FtDCn3CNpnvJ&offset=' + offset + '&limit=3';
    ...
    for (let i = 0; i < amount; i++) {
        $.ajax({url: query, method: 'GET'}).then(function(response){
            ...
            //Giphy doesnt return a rating with thier random search! How great. Lets make a second ajax call
            //Obviously this isnt ideal. Since the giphy api is on github I would probably request/fix this myself.
            //If given more time anyways.

            $.ajax({url: 'http://api.giphy.com/v1/gifs/' + response.data.id +  '?api_key=ihqeBjzMFNuSM45AZMn5FtDCn3CNpnvJ', method: 'GET'}).then(function(SecondResponse) {
